...
const AVAILABLE_FONTS = [
    'Arial', 'Times New Roman', 'Georgia', 'Verdana', 'Courier New', 'Impact', 'Trebuchet MS', 'Oswald', 'Cinzel'
];
...
const TextPropertiesPanel: React.FC = () => {
    ...
    if (selectedNodeId === 'nombres') { baseText = data.nombres; baseFont = m.nombres.fontFamily; baseIsBold = m.nombres.fontStyle?.includes('bold') || false; baseIsItalic = m.nombres.fontStyle?.includes('italic') || false; }
    else if (selectedNodeId === 'apellidos') { baseText = data.apellidos; baseFont = m.apellidos.fontFamily; baseIsBold = m.apellidos.fontStyle?.includes('bold') || false; baseIsItalic = m.apellidos.fontStyle?.includes('italic') || false; }
    else if (selectedNodeId === 'fechas') { baseText = `${data.fecha_nacimiento}  —  ${data.fecha_fallecimiento}`; baseFont = m.fechas.fontFamily; baseIsBold = m.fechas.fontStyle?.includes('bold') || false; baseIsItalic = m.fechas.fontStyle?.includes('italic') || false; }
    else if (selectedNodeId === 'velatorio_lugar') { baseText = data.velatorio.lugar; baseFont = m.velatorio_lugar.fontFamily; }
    else if (selectedNodeId === 'velatorio_direccion') { baseText = data.velatorio.direccion; baseFont = m.velatorio_direccion.fontFamily; }
    else if (selectedNodeId === 'sepelio_fecha_hora') { baseText = `${data.sepelio.fecha}  |  ${data.sepelio.hora}`; baseFont = m.sepelio_fecha_hora.fontFamily; }
    else if (selectedNodeId === 'sepelio_lugar') { baseText = data.sepelio.lugar; baseFont = m.sepelio_lugar.fontFamily; }
    else if (selectedNodeId === 'frase_dedicatoria') { baseText = data.frase_dedicatoria; baseFont = m.frase_dedicatoria.fontFamily; }
    ...
    return (
        <div className="bg-sky-50/50 border border-sky-100 p-4 rounded-xl space-y-4 shadow-inner my-4">
            ...
            {/* Content Edit */}
            <div>
                <label className="block text-[10px] font-semibold text-gray-500 mb-1">Contenido Manual</label>
                <textarea
                    value={currentText}
